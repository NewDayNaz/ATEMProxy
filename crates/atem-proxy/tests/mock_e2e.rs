...
use atem_protocol::{
    ...
    encode_packet, is_handshake_packet, parse_commands, serialize_command, PacketFlags,
    PacketHeader, CommandName, HANDSHAKE_OK_STATUS, ATEM_UDP_PORT,
};
...
async fn mock_atem(sock: UdpSocket) {
    ...
    loop {
        ...
        if hdr.flags.contains(PacketFlags::ACK_REQUEST) && !hdr.flags.contains(PacketFlags::HANDSHAKE)
        {
...
async fn two_clients_receive_init_dump_from_proxy() {
    ...
    cfg.atem = atem_addr.to_string();
    cfg.bind = proxy_addr;
    cfg.log = "error".into();
    cfg.client_idle_ms = 5000;
